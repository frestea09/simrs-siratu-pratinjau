...
export default function ImpuPage() {
  ...
  const impuIndicators = React.useMemo(() => {
    const filteredByCategory = indicators.filter(i => i.category === "IMPU")
    if (userIsCentral) {
  ...
  const indicatorsForUnit = React.useMemo(() => {
    ...
    return impuIndicators.filter(i => i.unit === selectedUnit)
  ...
  const meetingStandard = impuIndicators.filter(
  ...
  const getChartDescription = () => {
    const baseDescription =
      selectedIndicator === "Semua Indikator"
        ? "Menampilkan rata-rata capaian semua indikator IMPU."
